...
static inline bool simp_ioctl_valid(unsigned int cmd)
{
        switch (cmd) {
        case TEST_01:
        case TEST_02:
...
static int simp_ioctl(struct block_device *bdev, fmode_t mode,                    
                        unsigned int cmd, unsigned long arg)                    
{ 
        //void __user *argp = (void __user*)arg;
        //struct dummy_device *ddev = NULL;

        if(!simp_ioctl_valid(cmd))
                return -ENOTTY;
...
struct _ioblk *dev_lookup_blk(struct dummy_device *dev, sector_t sector)
{
        pgoff_t idx = sector >> PAGE_SECTORS_SHIFT;
        struct _ioblk *blk = NULL;
...
struct _ioblk* dev_insert_page(struct dummy_device *ddev, sector_t sector)
{
        struct _ioblk *blk = NULL;
        pgoff_t idx;
        ...
        blk = kmem_cache_zalloc(_cache_blk, GFP_KERNEL);
        if (!blk){
                goto err_alloc;
        }
        blk->page = alloc_page(GFP_KERNEL);
        if (!blk->page) {
                kmem_cache_free(_cache_blk, blk);
                goto err_alloc;
        }
...
err_alloc:
        printk("err alloc _ioblk.\n");
        return NULL;
}

/* if already dirty, return 0, else dirty it and return 1 */
int set_blk_dirty(struct _ioblk *blk)
{
...
int set_blk_tsync(struct _ioblk *blk)
{
	struct dummy_device *ddev = blk->ddev;

	atomic_inc(&ddev->tsync_blks);		

	write_lock_irq(&ddev->tree_lock);
	radix_tree_tag_set(&ddev->dev_pages, blk->index, RADIX_TREE_TAG_TSYNC);
	write_unlock_irq(&ddev->tree_lock);
...
static int __init radix_block_init(void)
{
	...
	_cache_blk = kmem_cache_create("ioblk", sizeof(struct _ioblk),
			0, 0, NULL);
	if (!_cache_blk) {
		ret = -ENOMEM;
		goto err_kmem_cache;
...
unsigned find_get_blks_tag(struct dummy_device *ddev, pgoff_t *index, 
		int tag, unsigned int nr_pages, struct _ioblk **blks)
{
...
void find_tag_blks(struct dummy_device *ddev, unsigned int tag)
{
	struct _ioblk *blks[BLKS_VEC_SIZE];
	...
	while (index <= end && (nr_gets = find_get_blks_tag(ddev, 
		&index, tag, BLKS_VEC_SIZE, blks)) > 0) {
		for (i = 0; i < nr_gets; i++) {
			struct _ioblk *blk = blks[i];
        ...
        }else if(tag == RADIX_TREE_TAG_TSYNC){
                printk("find tsync blks: %u\n", cnt);
                printk("tsync blks cnt = %u\n", atomic_read(&ddev->tsync_blks));
                printk("tsync write cnt = %u\n", atomic_read(&ddev->write_blks));
                printk("tsync read cnt = %u\n", atomic_read(&ddev->read_blks));
        }
}
			...
			if (!ret || ret != blks[i]) {
				pr_err("[%s]-[%d], pos:\n", 
	                ...
	                __free_pages(blks[i]->page, 0);
                        kmem_cache_free(_cache_blk, blks[i]);
		}
		pos++;
                blk_cnt += nr_pages;
	};
...
{
        printk("In exit\n");
        find_tag_blks(blk_dev, RADIX_TREE_TAG_DIRTY);
        find_tag_blks(blk_dev, RADIX_TREE_TAG_TSYNC);
        printk("\n");
        free_all_blks(blk_dev);
	kmem_cache_destroy(_cache_blk);
        kfree(blk_dev);
        del_gendisk(simp_blkdev_disk);
        put_disk(simp_blkdev_disk);
        blk_cleanup_queue(simp_blkdev_queue);
